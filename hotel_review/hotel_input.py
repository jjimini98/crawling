...
data = pd.read_csv("C:/Users/jennalee/Desktop/hotel.csv")
...
# 새로운 파일 hotel_afinn.csv 생성 후 한 문장씩 읽어들임.
# 맨 처음 실행해서 파일을 만들고 그다음부터는 그냥 주석처리해서 쓰심 됩니다
#re_data= open("C:/Users/jennalee/Desktop/hotel_afinn.csv", "w")
#for x in data['total_review']:
...
#scoredata = pd.read_csv("C:/Users/jennalee/Desktop/hotel_afinn.csv")
afinn = Afinn()

# 근데 결과가 이상함 다 -4.0 이 나온다.?? 한 번 파일을 확인해봐야한다.
f = open("C:/Users/jennalee/Desktop/hotel_input.csv", "w")
score = []
for x in  data['total_review']:
    score = str(afinn.score(x))
    f.write(x + "," + score + "\n")
f.close()
